...
export class MyapiService {
  ...
  CardbaseUrl="http://localhost:8585/cardapi/updatebal/"
  //RETRIEVING ALL PRODUCTS
  getAllProducts()
  {
    return this.httpClient.get(this.baseUrl+"/order/getAll");
  }

  serachProductById(productid:number)
  {
    return this.httpClient.get(this.baseUrl+"/order/getProductById/"+productid);
  }
  serachProductByName(pname:string)
  {
    return this.httpClient.get(this.baseUrl+"/order/getProductsByName/"+pname);
  }
  addNewProduct(product:Producthistory)
  {
    return this.httpClient.post("http://localhost:8585/order/addProducts",product);
  }
  ...
  baseUrlCard: string = 'http://localhost:8585/emicard';
  getAllEmiCard() {
    return this.httpClient.get(this.baseUrlCard + '/cards');
  }

  baseUrlUser: string = 'http://localhost:8585/user';
  ...
  addNewUser(user: User) {
    return this.httpClient.post(this.baseUrlUser + '/addUser', user);
  }
  ...
  baseUrlR:string="http://localhost:8585/finance";
  ...
  baseUrlS:string="http://localhost:8585/admin/validate";
  getAdmin(name:string,pass:string){
    return this.httpClient.get(this.baseUrlS+ name+"/"+pass);
    
  }
  baseUrlSB:string="http://localhost:8585/UserDetails"
  
  
  getAllUsers()
  {
    return this.httpClient.get(this.baseUrlSB+"/getUserdetails")
  }
  // getAllCards(){
  ...
  {
    return this.httpClient.put(this.baseUrlSB+"/findUser/"+regid,regid);
  }
  updateTheUser(regid:any,users:UserDetails)
  {
 
    return this.httpClient.put("http://localhost:8585/UserDetails/updateUser/"+regid,users);
  }
  deleteTheUser(regid:any,users:UserDetails)
  {
 
    return this.httpClient.delete("http://localhost:8585/UserDetails/deleteUser/"+regid);
  }
  searchUserById(regid:number)
  {
    return this.httpClient.get(this.baseUrlSB+"/findUser/"+regid);
  }
  addCard(card:Carddetails)
  {
    return this.httpClient.post("http://localhost:8585/UserDetails/addCard",card);
  }  
  getAllCards(){
    return this.httpClient.get(this.baseUrlSB+"/getCarddetails")
  }
  baseUrlRP:string="http://localhost:8585/order";
 
 
 addProducts(finProduct:Product){
   return this.httpClient.post(this.baseUrl+"/addProducts",finProduct);
